...
const LoginDelivery_OTP = () => {
    ...
    useEffect(() => {
        // Only send OTP if formData is available
        if (formData?.email) {
            sendOtp(); // Send OTP on component mount
        }

        // return () => {
        //     if (timerId) clearInterval(timerId); // Clear interval on component unmount
        // };
    }, [formData]); // Adding formData to the dependency array
    ...
    const sendOtp = async () => {
        try {
            ...
            if (response.status === 200) {
                ...
                // Start the countdown only after OTP is sent
