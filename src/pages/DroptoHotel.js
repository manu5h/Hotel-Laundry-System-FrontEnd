...
import BG_img from "../assets/images/droptohotel_bg.png";
...
const DroptoHotel = () => {
  ...
  const handleAccept = async (orderId) => {
    const confirmed = window.confirm(
      "Are you prepared to drop off this order at the Hotel? Please confirm."
    );
    if (!confirmed) return;

    try {
      const response = await fetch(
        `http://localhost:5000/order/${orderId}/complete`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${storedToken}`,
  ...
  return (
    ...
    >
      ...
      <div className="rider-process-details">
        <h2 style={{ marginBottom: "0" }}>Drop to Hotel</h2>
